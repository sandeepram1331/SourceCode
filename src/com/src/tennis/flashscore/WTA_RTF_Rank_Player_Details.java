...
public class WTA_RTF_Rank_Player_Details {

	public static void main(String args[]) throws IOException
	{
		Session session1 = AnnotationUtil.giveSession();
		Session session = AnnotationUtil.giveSession();
		File input = new File("E:\\d_drive\\WTARankings.html");
		Document doc = Jsoup.parse(input, "UTF-8", "http://example.com/");
		Element table = doc.getElementById("ranking-table-results_35853");
		...
		List<WTAPlayer> atpLi = new ArrayList<WTAPlayer>();
		List<String> urls = new ArrayList<String>();
		for(int k=0;k<max;k++)
		{
			WTAPlayer atp = new WTAPlayer();
